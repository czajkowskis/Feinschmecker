...
Queries search knowledge graphs for individuals. Most functions accept an optional
`kg` parameter to query specific knowledge graphs.
"""

from tornado.web import MissingArgumentError
from tornado.gen import UnknownKeyError
from owlready2 import Thing
from .setup import kg_onto
...
def getRecipe(recipe_name: str = None, title: str = None, kg=None) -> Recipe:
    """
    ...
    if title is not None:
        recipe_tmp = kg.search(type=Recipe, has_recipe_name=title)
        if len(recipe_tmp) != 0:
            recipe = recipe_tmp[0]
    if recipe is not None and recipe_name is not None and kg[recipe_name] is not None and type(kg[recipe_name]) is Recipe:
        recipe = kg[recipe_name]
    if recipe is None:
...
# Selection questions
def requiredIngredients(recipe: Recipe = None, title: str = None, kg=None) -> list[str]:
    """
    ...
    return getAll(getRecipe(title=title, kg=kg).has_ingredient, "has_ingredient_with_amount_name")


def recipesWithMaxCalories(amount: float = 0, kg=None) -> list[Recipe]:
    """
    ...
    if kg is None:
        kg = kg_onto
    return getAll(filter(lambda x: (x if x.amount_of_calories[0] <= amount else None), kg.search(type=Calories)),
                  "calories_of")


def recipesWithMinCalories(amount: float = 0, kg=None) -> list[Recipe]:
    """
...
def recipesWithMaxProtein(amount: float = 0, kg=None) -> list[Recipe]:
    """
    ...
    if kg is None:
        kg = kg_onto
    return getAll(filter(lambda x: (x if x.amount_of_protein[0] <= amount else None), kg.search(type=Protein)),
                  "protein_of")


def recipesWithMinProtein(amount: float = 0, kg=None) -> list[Recipe]:
    """
...
def recipesWithMaxFat(amount: float = 0, kg=None) -> list[Recipe]:
    """
    ...
    if kg is None:
        kg = kg_onto
    return getAll(filter(lambda x: (x if x.amount_of_fat[0] <= amount else None), kg.search(type=Fat)), "fat_of")


def recipesWithMinFat(amount: float = 0, kg=None) -> list[Recipe]:
    """
...
def recipesWithMaxCarbohydrates(amount: float = 0, kg=None) -> list[Recipe]:
    """
    ...
    if kg is None:
        kg = kg_onto
    return getAll(
        filter(lambda x: (x if x.amount_of_carbohydrates[0] <= amount else None), kg.search(type=Carbohydrates)),
        "carbohydrates_of")


def recipesWithMinCarbohydrates(amount: float = 0, kg=None) -> list[Recipe]:
    """
...
def recipesWithMaxTime(amount: float = 0, kg=None) -> list[Recipe]:
    """
    ...
    if kg is None:
        kg = kg_onto
    return getAll(filter(lambda x: (x if x.amount_of_time[0] <= amount else None), kg.search(type=Time)),
                  "time_required_by")


# Binary questions
def isVegan(recipe: Recipe = None, title: str = None, kg=None) -> bool:
    """
    ...
    return getRecipe(title=title, kg=kg).is_vegan[0]


def isVegetarian(recipe: Recipe = None, title: str = None, kg=None) -> bool:
    """
    ...
    return getRecipe(title=title, kg=kg).is_vegetarian[0]


def getDifficulty(recipe: Recipe = None, title: str = None, kg=None) -> int:
    """
    ...
    return getRecipe(title=title, kg=kg).has_difficulty[0].has_numeric_difficulty[0]


# Counting questions
def breakfastRecipesCount(kg=None) -> int:
    """
    ...
    if kg is None:
        kg = kg_onto
    return len(kg.search(type=MealType, has_meal_type_name="Breakfast")[0].meal_type_of)


def lunchRecipesCount(kg=None) -> int:
    """
    ...
    if kg is None:
        kg = kg_onto
    return len(kg.search(type=MealType, has_meal_type_name="Lunch")[0].meal_type_of)


def dinnerRecipesCount(kg=None) -> int:
    """
    ...
    if kg is None:
        kg = kg_onto
    return len(kg.search(type=MealType, has_meal_type_name="Dinner")[0].meal_type_of)
