...
from backend.app.utils.validators.recipe_validator import validate_recipe_filters, ValidationError
...
def get_recipes():
    ...
    try:
        ...
        per_page = validated_filters.pop('per_page', current_app.config['DEFAULT_PAGE_SIZE'])
        ...
        logger.info(f"Returning {len(recipes)} recipes (page {page}/{total_count // per_page + 1})")
        ...
        return internal_error_response(f"An error occurred while processing your request")
